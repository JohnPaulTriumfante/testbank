...
def add_question():
    ...
    return render_template("add_question.html", existing_problems=existing_problems)
...
def preview_exam():
    ...
    # If we don't have pre-selected questions, generate new ones
    ...
    if file_format == 'docx':
        ...
        save_exam_to_history(conn, int(chapter_id), docx_filename, docx_path, num_questions, include_problems)
        conn.close()
    ...
    # Save to exam history
    save_exam_to_history(conn, int(chapter_id), pdf_filename, pdf_path, num_questions, include_problems)
    conn.close()
